...
type LeadStatus =
  ...
  | "converted"
  | "lost";
type LeadSource = "website" | "referral" | "social" | "cold-call" | "event" | "ad";
...
const statusColors: Record<LeadStatus, { bg: string; text: string; ring: string }> =
  {
    new: {
      bg: "bg-blue-500/15",
...
export default function LeadsPage() {
  ...
  // Mock data - lots of leads!
  const initialLeads: Lead[] = useMemo(
    () => [
      {
        id: "l1",
        name: "Sarah Martinez",
        ...
        notes: "Interested in enterprise package. Needs demo by Friday.",
        starred: true,
      },
      {
        id: "l2",
      ...
      },
      {
        ...
        notes: "Very interested. Sent proposal, waiting for approval.",
        starred: true,
      },
      {
        id: "l4",
        name: "David Rodriguez",
        email: "d.rodriguez@business.com",
      ...
      },
      {
        ...
        notes: "Successfully converted! Onboarding scheduled.",
        starred: false,
      },
      {
        id: "l6",
      ...
      },
      {
        ...
        source: "website",
        value: 18000,
        dateAdded: new Date(Date.now() - 5 * 60 * 60 * 1000).toISOString(),
        notes: "Filled out contact form. Very interested.",
        starred: true,
      },
      {
        id: "l8",
        ...
        notes: "Sent initial information. Awaiting response.",
        starred: false,
      },
      {
        id: "l9",
        name: "Amanda White",
        email: "awhite@consulting.com",
        ...
        lastContact: new Date(Date.now() - 1 * 24 * 60 * 60 * 1000).toISOString(),
        notes: "Met at conference. Strong lead.",
        starred: true,
      },
      {
        id: "l10",
        ...
        source: "social",
        value: 7500,
        dateAdded: new Date(Date.now() - 8 * 60 * 60 * 1000).toISOString(),
        notes: "LinkedIn outreach. Interested in demo.",
        starred: false,
      },
      {
        id: "l11",
        name: "Lisa Patel",
        email: "lpatel@healthcare.com",
        ...
        notes: "Large deal. Reviewing final proposal.",
        starred: true,
      },
      {
        id: "l12",
  ...
  // --- NEW: make leads mutable so purchases can add leads into the table
  ...
  const handleAiCall = (leadId: string, leadName: string) => {
    ...
    alert(
      `🤖 AI Call initiated for ${leadName}\n\n(This is UI only - connect to your AI calling system)`
    );
  ...
  // --- NEW: fake lead generator (placeholder until you wire to backend / Stripe)
  function generatePurchasedLeads() {
    ...
    const newOnes: Lead[] = Array.from({ length: buyForm.quantity }).map((_, i) => {
      const t = templates[i % templates.length];
      const id = `p_${Date.now()}_${i}`;
      const name = `${t.first} ${t.last}`;
      const email = `${t.first.toLowerCase()}.${t.last.toLowerCase()}${i}@example.com`;
      const phone = `+1 (555) ${String(100 + (i % 900)).padStart(3, "0")}-${String(
        1000 + (i % 9000)
      ).padStart(4, "0")}`;

      // Roughly consistent value by pack type
      const value =
      ...
      return {
        ...
        notes:
          buyForm.type === "high-intent"
            ? "High-intent lead (placeholder). Wire to backend for real data."
            : "Verified lead (placeholder). Wire to backend for real data.",
  ...
  const handlePurchase = () => {
    ...
    setLeads((prev) => [...newLeads, ...prev]); // add to top

    alert(
      `✅ Purchased ${buyForm.quantity} ${buyForm.type === "high-intent" ? "High Intent" : "Verified"} leads\n` +
        `Industry: ${buyForm.industry}\n` +
        `City: ${buyForm.city.trim() || "Selected City"}\n` +
        `Total: $${totalPrice}\n\n` +
        `(UI only) Next: wire to Stripe + /api/leads/purchase and deliver real leads to MongoDB.`
    );
      ...
      <div className="mx-auto w-full max-w-[1600px] px-6 py-8 flex flex-col flex-1">
        ...
        <div className="flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          ...
          <div className="flex gap-2 flex-wrap">
            ...
            {/* Your old "Add Lead" was here; leaving it out since you didn’t show handler */}
            <button className={btn("ghost")}>
        ...
        <div className="mt-6 grid grid-cols-2 lg:grid-cols-4 gap-4">
          <div className={cx(panel, "p-4")}>
            <div className="flex items-center justify-between">
              <div>
                <div className={cx("text-xs font-semibold", isDark ? "text-zinc-400" : "text-neutral-600")}>
                  Total Leads
                </div>
                <div className={cx("text-2xl font-bold mt-1", isDark ? "text-white" : "text-neutral-900")}>
                  {stats.total}
                </div>
              </div>
              <Users className={cx("w-8 h-8", isDark ? "text-zinc-600" : "text-neutral-300")} />
            </div>
          </div>
          ...
          <div className={cx(panel, "overflow-hidden flex-1 flex flex-col")}>
            <div className="overflow-x-auto flex-1">
              <table className="w-full">
                <thead className={cx("border-b", isDark ? "border-white/10" : "border-neutral-200")}>
                  <tr>
                    <th className={cx("text-left px-4 py-3 text-xs font-semibold", isDark ? "text-zinc-400" : "text-neutral-600")}>
                      Lead
                    </th>
                    <th className={cx("text-left px-4 py-3 text-xs font-semibold", isDark ? "text-zinc-400" : "text-neutral-600")}>
                      Contact
                  ...
                  {filteredLeads.length === 0 ? (
                    <tr>
                      <td colSpan={7} className="px-4 py-12 text-center">
                        <div className={cx("text-sm", isDark ? "text-zinc-400" : "text-neutral-600")}>
                    ...
                    filteredLeads.map((lead) => (
                      <tr
                        ...
                        className={cx(
                          ...
                          isDark ? "border-white/5 hover:bg-white/[0.02]" : "border-neutral-100 hover:bg-neutral-50"
                        )}
                      >
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-3">
                            {lead.starred && <Star size={14} className="text-amber-500 fill-amber-500" />}
                            <div>
                              <div className={cx("font-semibold text-sm", isDark ? "text-white" : "text-neutral-900")}>
                                {lead.name}
                              </div>
                              {lead.company && (
                                <div className={cx("text-xs mt-0.5", isDark ? "text-zinc-400" : "text-neutral-600")}>
                                  {lead.company}
                                </div>
                              )}
                              <div className={cx("text-xs mt-0.5 flex items-center gap-1", isDark ? "text-zinc-500" : "text-neutral-500")}>
                                <MapPin size={10} />
                                {lead.location}
                              </div>
                            </div>
                        ...
                        <td className="px-4 py-4">
                          <div className="space-y-1">
                            <div className={cx("text-xs flex items-center gap-1.5", isDark ? "text-zinc-300" : "text-neutral-700")}>
                              <Mail size={12} />
                              {lead.email}
                            </div>
                            <div className={cx("text-xs flex items-center gap-1.5", isDark ? "text-zinc-300" : "text-neutral-700")}>
                        ...
                        <td className="px-4 py-4">
                          <div className={cx("text-sm font-semibold", isDark ? "text-white" : "text-neutral-900")}>
                            ${fmtMoney(lead.value)}
                          </div>
                        </td>

                        <td className="px-4 py-4">
                          <div className={cx("text-xs capitalize", isDark ? "text-zinc-400" : "text-neutral-600")}>
                            {lead.source.replace("-", " ")}
                          </div>
                        </td>

                        <td className="px-4 py-4">
                          <div className={cx("text-xs flex items-center gap-1.5", isDark ? "text-zinc-400" : "text-neutral-600")}>
                            <Clock size={12} />
                            {lead.lastContact
                              ? new Date(lead.lastContact).toLocaleDateString(undefined, {
                                  month: "short",
                                  day: "numeric",
                                })
                              : "Never"}
                        ...
                        <td className="px-4 py-4">
                          <div className="flex items-center gap-2">
                            ...
                            >
                              <PhoneCall size={14} className={callingLeadId === lead.id ? "animate-pulse" : ""} />
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className={cx("w-full max-w-lg rounded-2xl p-5 md:p-6", panel)}>
            <div className="flex items-start justify-between gap-4">
              <div>
                <h2 className={cx("text-xl font-bold", isDark ? "text-white" : "text-neutral-900")}>
                  Buy Leads
                </h2>
                <p className={cx("mt-1 text-sm", isDark ? "text-zinc-400" : "text-neutral-600")}>
            ...
            <div className="mt-5 space-y-4">
              <div>
                <div className={cx("text-xs font-semibold mb-1", isDark ? "text-zinc-400" : "text-neutral-600")}>
                  Industry
                </div>
                <select
                  className={input}
                  value={buyForm.industry}
                  onChange={(e) => setBuyForm((p) => ({ ...p, industry: e.target.value as LeadIndustry }))}
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setBuyForm((p) => ({ ...p, quantity: Number(e.target.value) as any }))}
                  >
                ...
                <div>
                  ...
                  <select
                    className={input}
                    value={buyForm.type}
                    onChange={(e) => setBuyForm((p) => ({ ...p, type: e.target.value as LeadPackType }))}
                  >
                    <option value="verified">Verified ($0.20/lead)</option>
                    <option value="high-intent">High Intent ($0.35/lead)</option>
                  </select>
                </div>
              </div>
              ...
              <div
                className={cx(
                  ...
                  isDark ? "bg-white/5 ring-white/10" : "bg-neutral-50 ring-neutral-200"
                )}
              >
                <div>
                  <div className={cx("text-xs font-semibold", isDark ? "text-zinc-400" : "text-neutral-600")}>
                  ...
                  <div className={cx("text-xs mt-1", isDark ? "text-zinc-500" : "text-neutral-500")}>
                    {buyForm.quantity} leads × ${pricePerLead.toFixed(2)}/lead
                  </div>
                </div>
                <div className={cx("text-2xl font-bold", isDark ? "text-white" : "text-neutral-900")}>
                  ${totalPrice.toFixed(2)}
                </div>
              </div>

              <div className="flex gap-2 pt-1">
                <button onClick={() => setShowBuyModal(false)} className={btn("ghost") + " w-full"}>
                  Cancel
                </button>
              ...
              <div className={cx("text-xs leading-relaxed", isDark ? "text-zinc-500" : "text-neutral-500")}>
                This is currently <span className="font-semibold">UI-only</span>. Next step is wiring this to{" "}
                <span className="font-semibold">Stripe</span> and an{" "}
                <span className="font-semibold">/api/leads/purchase</span> route that writes leads to MongoDB under the
                tenant/business.
              </div>
            </div>
          </div>
