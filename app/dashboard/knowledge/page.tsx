...
export default function KnowledgeBasePage() {
  ...
  useEffect(() => {
    (async () => {
      ...
      try {
        ...
        const sec = kb.sections;
        if (sec?.builtins) setBuiltins(sec.builtins);
        if (Array.isArray(sec?.customs)) setCustomSections(sec.customs);
  ...
  // derived rawText (for preview only)
  ...
  function deleteCustom(id: string) {
    ...
    if (active === (`custom:${id}` as SectionId)) setActive("builtin:services");
  }
      ...
      <div className="max-w-[1500px] mx-auto p-8">
        <div className="mb-8">
          ...
          <p className={`mt-2 text-base leading-relaxed ${subtle}`}>
            Build your receptionist’s “brain” in sections. Saved to Mongo as JSON.
          </p>
        </div>
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className={`mt-1 text-sm ${subtle}`}>
                    Derived from JSON sections (stored canonically).
                  </p>
                </div>
                <span
