...
export default function PayPalConnectButton({
  ...
  const handleConnect = async () => {
    ...
    try {
      ...
      const res = await fetch("/api/payments/paypal/connect", {
        method: "POST",
      ...
      if (data.success && data.url) {
        ...
        alert(data.error || "Failed to connect PayPal");
        setLoading(false);
      }
    } catch (error) {
      console.error("PayPal connect error:", error);
      alert("Failed to connect PayPal");
      setLoading(false);
    }
  };

  const handleDisconnect = async () => {
    if (!confirm("Are you sure you want to disconnect PayPal?")) {
      return;
    }

    setLoading(true);
    try {
      const res = await fetch("/api/payments/paypal/disconnect", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ businessId }),
      });
  ...
  const handleManage = () => {
    // Link to PayPal Dashboard
    window.open("https://www.paypal.com/businessmanage/account/aboutBusiness", "_blank");
  };
